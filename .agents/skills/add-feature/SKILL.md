...
2. **其他任何形式**（自由文本、中文、含空格/特殊字符）：视为自由描述。
   - 由你（Codex）基于描述生成一个 **ASCII kebab-case slug** 候选：
...
- 新增的**控制器/模块**归属：参照 AGENTS.md 里 `background/` 各 controller 的职责划分，不要把"模板 + DOM + 业务规则 + 消息派发"塞进同一个文件。
...
- **前端改动**：按 AGENTS.md 要求，启动 dev server 在浏览器里手测 golden path 和边界，观察其它功能是否回归；类型检查和测试只验证代码正确性，不验证功能正确性。
